...
if (!exists("healthy_data_pancreas_mb")) {
  healthy_data_pancreas_mb <- read.delim("data/healthy_transcripts.tsv", header = TRUE, sep = "\t", row.names = 1)
}

if (!exists("cancer_mb")) {
  cancer_mb <- read.delim("data/cancer_mb_transcripts.tsv", header = TRUE, sep = "\t", row.names = 1)
}
...
write.csv(filtered_res_signif, "data/filtered_deseq2_transcripts_results_w_log.csv", row.names = TRUE)
